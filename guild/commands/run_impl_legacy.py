...
def _apply_run_params(run, args):
    ...
    for name, val in run_params.items():
        ...
        if (name == "no_gpus" and args.gpus) or (name == "gpus" and args.no_gpus):
            continue
...
def resolve_model_op(opspec):
    try:
        ...
        opdef = (
            model.modeldef.get_operation(op_name)
            if op_name
...
def _resolve_model(model_ref):
    return util.find_apply(
        [_resolve_cwd_model, _resolve_system_model, _maybe_no_model_error,], model_ref
    )
...
def _op_label(args, opdef, flag_vals):
    ...
    if opdef.batch_opspec:
        return None
    return _current_label(args, flag_vals) or op_util.default_label(opdef, flag_vals)
...
def _restart_or_restage_run(args):
    return getattr(args, "_restart_run", None) or getattr(args, "_restage_run", None)
...
def _apply_optimizer_and_flags(optdef, args):
    ...
    default_opt_flags = tuple(
        [op_util.flag_assign(name, val) for name, val in sorted(optdef.flags.items())]
    )
...
def _apply_arg_objective(args, opdef):
    if args.minimize:
        ...
        opdef.objective = {"maximize": args.maximize}
...
def _resolve_batch_opdef(batch_opspec):
    try:
        ...
        cli.error(
            ...
            "verify a custom optimizer by running "
            "'guild operations'." % batch_opspec
        )
    else:
...
def _apply_batch_flag_encoder(op, user_flags):
    ...
    for flag_name in op.flag_vals:
        ...
        if not flagdef:
            continue
        op.flag_vals[flag_name] = encode_flag_val(op.flag_vals[flag_name], flagdef)
...
def _read_batches(path):
    ...
    if ext in (".json", ".yml", ".yaml"):
        ...
        cli.error("unsupported batch file extension for %s" % path)


def _yaml_batches(path):
    data = yaml.safe_load(open(path, "r"))
    if not isinstance(data, list):
        cli.error("unsupported data type for batch file %s: %s" % (path, type(data)))
    for item in data:
        if not isinstance(item, dict):
            cli.error("supported data for batch file %s trial: %r" % item)
    return data
...
def _find_matching_runs(op):
    ...
    matching = [
        run
        for run in resolver.matching_runs([to_match.opref])
...
def _match_run_flags(run, target):
    ...
    log.debug("comparing run %s flags %r to target %r", run.id, run_flags, target)
    return run_flags == target


def _match_proto_run(batch_run, proto_op):
    proto_run = runlib.Run("", batch_run.guild_path("proto"))
    return proto_op.opref.is_op_run(proto_run) and _match_run_flags(
        proto_run, proto_op.flag_vals
    )
