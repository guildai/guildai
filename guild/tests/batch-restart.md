...
## Restarting only when needed

When restarting a large number of runs, the `needed` option can be
used to skip restarting applicable trials that are already completed.

To illustrate we'll use two trials with restarts using the needed
option.

First, let's delete the existing runs:

    >>> project.delete_runs()
    Deleted 5 run(s)

Next we initialize 2 trials:

    >>> run("echo.py", x=[1,2], init_trials=True)
...
Our runs:
    ...
    >>> print_runs(status=True)
    echo.py   x=2 y=2 z=a  pending
    echo.py   x=1 y=2 z=a  pending
    echo.py+               pending

Let's restart the batch with the needed option:

    >>> batch_run = project.list_runs()[-1]
    >>> batch_run.opref.to_opspec()
    '+'

    >>> run(restart=batch_run.id, needed=True)
    Restarting ...
    Initialized trial (x=1, y=2, z=a)
    Initialized trial (x=2, y=2, z=a)
...
And another restart with needed:
