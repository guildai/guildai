...
# TODO:
#
# The scheme for importing flags needs to be generalized as a facility
# for dynamically generating or modifying Guild file data - in
# particular op data. It's fine to limit this facility to Python
# support for now as all of the dynamic functionality is for Python
# main modules.
#
# The facility should look like this:
#
# - Check a cache of cached data - this should not be limited to flag
#   data as it is now
#
# - If cache miss, load the Python script and consult plugins for some
#   interface - call the methods on that interface to collect data to
#   cache
#
# - Use cached data to form/reform the Guild file data / op data
...
class PythonScriptOpSupport(object):
    ...
    `python_script_op_data` is called to potentially update op data.
    """
...
class PythonScriptPlugin(pluginlib.Plugin):
    ...
    def guildfile_data(self, data, src):
        ...
        for op_data in self._iter_ops_with_main(data):
            ...
            self._notify_python_script_op(op_data, gf_dir)

    @staticmethod
    def _notify_python_script_op(op_data, gf_dir):
        for _name, plugin in pluginlib.iter_plugins():
            if isinstance(plugin, PythonScriptOpSupport):
                plugin.python_script_op_data(op_data, gf_dir)
    ...
    def _flags_data(self, main_mod, imports, gf_dir):
        try:
            sys_path, mod_path = self.find_module(main_mod, gf_dir)
    ...
    def find_module(self, main_mod, gf_dir):
