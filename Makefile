...
uat:
	@test -e $(guild-uat) || $(guild) init -p $(UAT_PYTHON) $(guild-uat) --pre -y
	@. $(guild-uat)/bin/activate && WORKSPACE=$(guild-uat) EXAMPLES=examples TEST_DEPS_OPTS='--pre' $(guild) check --uat --notify
